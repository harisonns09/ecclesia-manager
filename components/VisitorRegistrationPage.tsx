...
const VisitorRegistrationPage: React.FC = () => {
  ...
  const handleDateChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (error) setErrors(prev => ({ ...prev, dataNascimento: error }));
    else setErrors(prev => { const n = { ...prev }; delete n.dataNascimento; return n; });
  };
  ...
  const handlePhoneChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    else if (value.length > 6) formatted = value.replace(/^(\d{2})(\d{4})(\d{0,4})/, '($1) $2-$3');
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    if (dateError) {
        ...
        toast.error("Verifique a data de nascimento.");
        return;
    }
    setIsModalOpen(true);
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans">
      <ConfirmationModal
        isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onConfirm={handleConfirmAction}
        title="Confirmar Cadastro" isProcessing={isSubmitting} colorClass="blue"
        description={<>Você confirma o envio dos seus dados para a <strong>{church.name}</strong>?</>}
          ...
          <form onSubmit={handleFormSubmit} className="space-y-5">
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <input required className="input-field !pl-11 py-3.5 bg-gray-50 focus:bg-white" placeholder="Seu nome completo" value={formData.nome} onChange={e => setFormData({ ...formData, nome: e.target.value })} />
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <input required type="tel" className="input-field !pl-11 py-3.5 bg-gray-50 focus:bg-white" placeholder="(00) 00000-0000" value={formData.telefone} onChange={handlePhoneChange} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <input type="date" max={new Date().toISOString().split("T")[0]} className={`input-field !pl-11 py-3.5 bg-gray-50 focus:bg-white ${errors.dataNascimento ? '!border-red-500 !bg-red-50' : ''}`} value={formData.dataNascimento} onChange={handleDateChange} />
                  <Calendar size={20} className={`absolute left-3.5 top-3.5 ${errors.dataNascimento ? 'text-red-500' : 'text-gray-400'}`} />
                </div>
                {errors.dataNascimento && <p className="text-red-500 text-xs mt-1 flex items-center font-medium"><AlertCircle size={12} className="mr-1" /> {errors.dataNascimento}</p>}
              </div>
              <div className="space-y-1.5">
                <label className="text-sm font-bold text-gray-700 ml-1">Email (Opcional)</label>
                <div className="relative">
                  <input type="email" className="input-field !pl-11 py-3.5 bg-gray-50 focus:bg-white" placeholder="seu@email.com" value={formData.email} onChange={e => setFormData({ ...formData, email: e.target.value })} />
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <textarea rows={2} className="input-field !pl-11 py-3.5 bg-gray-50 focus:bg-white resize-none" placeholder="Ex: Convite de amigo, Instagram..." value={formData.observacao} onChange={e => setFormData({ ...formData, observacao: e.target.value })} />
                <MessageCircle size={20} className="absolute left-3.5 top-3.5 text-gray-400" />
              </div>
            </div>
            <button type="submit" disabled={isSubmitting} className="w-full py-4 bg-[#1e3a8a] text-white rounded-xl font-bold hover:bg-[#172554] transition-all shadow-lg hover:shadow-xl flex items-center justify-center mt-4 disabled:opacity-70">
              <Send size={20} className="mr-2" /> Enviar Cadastro
            </button>
            <p className="text-xs text-center text-gray-400 mt-4">Seus dados estão seguros conosco.</p>
          </form>
        </div>
      </div>
    </div>
