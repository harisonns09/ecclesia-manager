...
const Visitors: React.FC = () => {
  ...
  const handleConfirmDelete = async () => {
    if (!church || !visitorToDelete) return;
    setIsProcessingDelete(true);
    const toastId = toast.loading("Removendo...");
    try {
      await visitorApi.delete(church.id, visitorToDelete);
    ...
    return (
        <div className={`bg-white rounded-2xl border ${isMembro ? 'border-emerald-200' : 'border-gray-200'} shadow-sm hover:shadow-md transition-all flex flex-col overflow-hidden m-2`}>
            ...
            <div className="flex justify-end gap-1 p-2 bg-gray-50/50 border-t border-gray-100">
                ...
                <button onClick={() => handleDeleteClick(visitor.id)} className="p-1 text-red-600 hover:bg-red-100 rounded transition-colors"><Trash2 size={14} /></button>
            </div>
        </div>
    );
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 relative pb-10">
      <ConfirmationModal
        ...
        description="Tem certeza que deseja excluir este visitante? Esta ação não pode ser desfeita."
           ...
           <form onSubmit={handleSubmit} className="p-6 space-y-5">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <div className="relative">
                    <input required className="input-field pl-10" value={formData.telefone} onChange={e => setFormData({...formData, telefone: formatPhone(e.target.value)})} placeholder="(00) 00000-0000" maxLength={15} />
